...
# Monitor-Baba Server Agent Installer
...
echo -e "\\e[34m[+] Starting Monitor-Baba Agent Installation...\\e[0m"

# Cleanup old 'mooonitooor' agent if it exists
if systemctl list-unit-files | grep -q "mooonitooor-agent.service"; then
    echo -e "\\e[33m[*] Migrating from old mooonitooor-agent to Monitor-Baba-agent...\\e[0m"
    sudo systemctl stop mooonitooor-agent 2>/dev/null || true
    sudo systemctl disable mooonitooor-agent 2>/dev/null || true
    sudo rm -f /etc/systemd/system/mooonitooor-agent.service
    sudo rm -rf /opt/mooonitooor-agent 2>/dev/null || true
...
INSTALL_DIR="/opt/Monitor-Baba-agent"
...
sudo cat <<EOF > /etc/systemd/system/Monitor-Baba-agent.service
[Unit]
Description=Monitor-Baba Monitoring Agent Daemon
...
sudo systemctl daemon-reload
sudo systemctl enable Monitor-Baba-agent
sudo systemctl restart Monitor-Baba-agent

echo -e "\\e[32m[✓] Monitor-Baba Agent successfully installed and running in the background!\\e[0m"
echo -e "You can check logs with: \\e[36mjournalctl -u Monitor-Baba-agent -f\\e[0m"
`;
